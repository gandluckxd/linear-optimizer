...
class GuillotineOptimizer:
    ...
    def _optimize_material(self, details: List[Detail], sheets: List[Sheet]) -> Tuple[List[SheetLayout], List[Detail]]:
        ...
        logger.info(f"🔄 МАКСИМАЛЬНОЕ использование {len(remainder_sheets)} остатков + {len(material_sheets)} цельных листов")
        ...
        if remainder_sheets:
            ...
            for sheet in remainder_sheets:
                ...
                if layout and len(layout.get_placed_details()) > 0:
                    ...
                    logger.info(f"✅ Остаток {sheet.id} использован: размещено {len(layout.get_placed_details())} деталей")
                else:
                    logger.info(f"⏭️ Остаток {sheet.id} не использован")
